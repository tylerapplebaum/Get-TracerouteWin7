...
Function script:Get-v2Traceroute {
	$script:TraceResults = @() #Store all hops in the path
	...
	Do {
		...
		Write-Verbose $ICMPResults.Address.IPAddressToString
		Write-Verbose $ICMPResults.Status
		$TraceResults += $ICMPResults.Address.IPAddressToString
		$zz++
	}
	Until ($ICMPResults.Status -eq "Success")
	#While ($ICMPResults.Status -ne "Success")
	Write-Verbose "Initial traceroute loop exited"
	Write-Verbose ($TraceResults | Out-String)
} #End Get-v2Traceroute
...
$script:HopName = New-Object psobject
  If ($Hop -notlike "TimedOut" -and $Hop -notlike "0.0.0.0") {
    ...
    $script:HopNameArr = [Net.DNS]::GetHostEntry($Hop) | Select -ExpandProperty HostName
